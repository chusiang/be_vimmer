...
CTRL-D          在當前行刪除一個 shiftwidth 的縮排
CTRL-E          插入游標下方的字元
...
'(              1  游標移動到當前句子的首部所在行的第一個 CHAR上
')              1  游標移動到當前句子的尾部所在行的第一個 CHAR上
'<              1  游標移動到當前緩衝區高亮區開始/曾經開始的行的第一個 CHAR 上。
'>              1  游標移動到當前緩衝區高亮區結束/曾經結束的行的第一個 CHAR 上。
'[              1  游標移動到當前緩衝區最近操作/放置的文本的首部所在行的第一個 CHAR 上
']              1  游標移動到當前緩衝區最近操作/放置的文本的尾部所在行的第一個 CHAR 上
'{              1  游標移動到當前段落的首部所在行的第一個 CHAR上
'}              1  游標移動到當前段落的尾部所在行的第一個 CHAR上
(               1  游標回退 N 個句子
)               1  游標前進 N 個句子
*               1  正向搜索第 N 次出現的游標所在的標識符
+               1  等同於 <CR>
...
H               1  游標移動到屏幕頂部開始算的第 N 行
I               2  在本行第一個 CHAR 之前插入文本 N 次
J               2  連接 (Join) N 行；預設為 2
K                  查找游標所在的關鍵字 (Keyword)，使用'keywordprg' 的定義
L               1  游標移動到屏幕底部開始算的第 N 行
M               1  游標移動到屏幕的中間一行
N               1  反方向重複最近的 '/' 或 '?' N 次
O               2  在游標上方開始一個新行，並插入文本，重複 N次
["x]P           2  在游標放置文本 [到緩衝區 (見上) x] N 次
Q                  切換到 "Ex" 模式
R               2  進入替換模式: 覆蓋存在的字元，重複輸入文本N-1 次
...
a               2  在游標處附加文本 N 次
b               1  游標反向移動 N 個單詞
["x]c{motion}   2  刪除 Nmove 文本 [到緩衝區 (見上) x] 並開始編輯
["x]cc          2  刪除 N 行 [到緩衝區 (見上) x] 並開始編輯
["x]d{motion}   2  刪除 Nmove 文本 [到緩衝區 (見上) x]
...
k               1  游標上移 N 行
l               1  游標右移 N 個字元
m{A-Za-z}          在游標位置處設置位置標記 {A-Za-z}
n               1  重複最近的 '/' 或 '?' N 次
o               2  在游標下方開始一個新行，並插入文本，重複 N次
["x]p           2  在游標後放置文本 [從暫存器 x] N 次
q{0-9a-zA-Z"}      記錄輸入的字元到命令暫存器 {0-9a-zA-Z"}(大寫用於添加)
q                  (在記錄時) 停止記錄
q:                 在命令行窗口裡編輯 : 命令行
q/                 在命令行窗口裡編輯 / 命令行
q?                 在命令行窗口裡編輯 ? 命令行
...
["x]y{motion}      抽出 Nmove 文本 [到緩衝區 (見上) x]
...
CTRL-W ]           分割窗口並跳轉到游標所在的標籤上
CTRL-W ^           分割窗口並編輯輪換文件 N
CTRL-W _           設置當前窗口高度為 N (預設: 儘量大)
CTRL-W b           跳轉到底部的窗口
CTRL-W c           關閉當前窗口 (類似於 |:close|)
CTRL-W d           分割窗口並跳轉到游標所在的定義上
CTRL-W f           分割窗口並跳轉到游標所在的文件名上
CTRL-W F           分割窗口並編輯游標所在的文件名，然後跳轉到文件名後給出的行號上。
CTRL-W g CTRL-]  分割窗口並在游標所在的標籤上執行 |:tjump|
CTRL-W g ]         分割窗口並在游標所在的標籤上執行 |:tselect|
CTRL-W g }         在游標所在的標籤上執行 |:ptjump|
CTRL-W g f         在新的標籤頁裡編輯游標所在的文件名
CTRL-W g F         在新的標籤頁裡編輯游標所在的文件名然後跳轉到文件名後給出的行號上。
CTRL-W h           跳轉到左面第 N 個的窗口 (在首個窗口上停止)
CTRL-W i           分割窗口並跳轉到游標所在的標識符的聲明上
...
CTRL-W x           交換當前和第 N 個窗口 (預設: 下一個窗口)
CTRL-W z           關閉預覽窗口
CTRL-W |           設置窗口寬度為 N 列
CTRL-W }           在預覽窗口裡顯示游標所在的標籤
CTRL-W <Down>      等同於 "CTRL-W j"
CTRL-W <Up>        等同於 "CTRL-W k"
CTRL-W <Left>      等同於 "CTRL-W h"
CTRL-W <Right>    等同於 "CTRL-W l"
###方括號命令
[ CTRL-D           跳轉到第一個在當前文件和它包含的頭文件裡匹配游標所在的單詞的 #define，從當前文件的頭部開始
[ CTRL-I           跳轉到第一個在當前文件和它包含的頭文件裡匹配游標所在單詞的位置，從當前文件頭部開始
[#              1  游標移動到前面第 N 個未匹配的 #if、#else 或者 #ifdef
['              1  游標移動到前面的小寫位置標記，定位在它所在行的第一個非空白字元上
[(              1  游標移動到前面第 N 個未匹配的 '('
[*              1  等同於 "[/"
[`              1  游標移動到前一個小寫位置標記
[/              1  游標移動到前面第 N 個 C 註釋的開始處
[D                 列出在當前文件和它包含的頭文件裡匹配游標所在的單詞的所有定義，從當前文件的頭部開始
[I                 列出在當前文件和它包含的頭文件裡匹配游標所在單詞的所有位置，從當前文件的頭部開始
[P              2  等同於 "[p"
[[              1  游標後退 N 個小節
[]              1  游標後退 N 個 SECTION
[c              1  游標後退 N 個更改開始的地方
[d                 顯示第一個在當前文件和它包含的頭文件裡匹配游標所在的單詞的 #define，從當前文件的頭部開始
[f                 等同於 "gf"
[i                 顯示第一個在當前文件和它包含的頭文件裡匹配游標所在單詞的位置，從當前文件的頭部開始
[m              1  游標後退 N 個成員函數的開始
[p              2  類似於 "P"，但調整當前行的縮排
[s              1  移動到前一個拼寫錯誤的單詞
[z              1  移動到打開的摺疊的開始
[{              1  游標後退 N 個未匹配的 '{'
[<MiddleMouse>  2  等同於 "[p"
] CTRL-D           跳轉到第一個在當前文件和它包含的頭文件裡匹配游標所在的單詞的 #define，從游標位置開始
] CTRL-I           跳轉到第一個在當前文件和它包含的頭文件裡匹配游標所在單詞的位置，從游標位置開始
...
]D                 列出在當前文件和它包含的頭文件裡匹配游標所在的單詞的所有定義，從游標位置開始
]I                 列出在當前文件和它包含的頭文件裡匹配游標所在單詞的所有位置，從游標位置開始
]P              2  等同於 "[p"
]]              1  游標前進 N 個小節
]c              1  游標前進 N 個更改開始的地方
]d                 顯示第一個在當前文件和它包含的頭文件裡匹配游標所在的單詞的 #define，從游標位置開始
]f                 等同於 "gf"
]i                 顯示第一個在當前文件和它包含的頭文件裡匹配游標所在單詞的位置，從游標位置開始
]m              1  游標前進 N 個成員函數的結尾
]p              2  類似於 "p"，但調整當前行的縮排
]s              1  移動到後一個拼寫錯誤的單詞
]z              1  移動到打開的摺疊的結尾處
...
g#              1  類似於 "#"，但不使用 "\<" 和 "\>"
g$              1  'wrap' 關閉時轉到當前行最右側屏幕上可見的字元'wrap' 打開時轉到當前屏幕行最右側的字元
g&              2  在所有行上重複上次的 ":s"
g'{mark}        1  類似於 |'|，但不改變跳轉表
g`{mark}        1  類似於 |`|，但不改表跳轉表
g*              1  類似於 "*"，但不使用 "\<" 和 "\>"
g0              1  'wrap' 關閉時轉到當前行最左側屏幕上可見的字元'wrap' 打開時轉到當前屏幕行最左側的字元
g8                 顯示游標所在的 UTF-8 字元的十六進制字節碼
g<                 顯示上次的命令輸出
g?              2  Rot13 編碼操作符
g??             2  Rot13 編碼當前行
g?g?            2  Rot13 編碼當前行
gD              1  轉到當前文件的游標所在單詞的定義
gE              1  反向轉移到上一個 WORD 的結尾處
gH                 啟動選擇行模式
gI              2  類似於 "I"，但總是從第一列開始
gJ              2  連接行，不插入空格
["x]gP          2  在游標籤放置文本 [從暫存器 x] N 次，游標留在插入文本之後
gR              2  進入虛擬替換模式
gU{motion}      2  使得 Nmove 文本全部大寫
gV                 在執行選擇模式下的映射或者選單時，不要重複選擇過去的可視區域
g]                 :tselect 到游標所在的標籤上
g^              1  'wrap' 關閉時轉到當前行最左側屏幕上可見的非空白字元'wrap' 打開時轉到當前屏幕行最左側的非空白字元
ga                 打印游標所在處的字元的 ascii 值
gd              1  轉到當前函數裡當前游標所在的單詞的定義
ge              1  反向轉到前一個單詞的結尾處
gf                 開始編輯當前游標所在的名字對應的文件
gF                 開始編輯游標所在的文件名並跳轉到文件名後的行號上。
...
["x]gp          2  在游標後面放置文本 [從暫存器 x] N 次，游標留在後面
gq{motion}      2  排版 Nmove 文本
gr{char}        2  虛擬替換 N 個字元為 {char}
gs                 睡眠 N 秒 (預設 1)
gu{motion}      2  使得 Nmove 文本全部小寫
gv                 重新選擇上次的可視區域
gw{motion}      2  排版 Nmove 文本並保持游標位置
gx                 執行游標下的文件名對應的應用程序 (僅限於|netrw| 插件)
g@{motion}         調用 'operatorfunc'
g~{motion}      2  改變 Nmove 文本的大小寫
g<Down>         1  等同於 "gj"
g<End>          1  等同於 "g$"
g<Home>         1  等同於 "g0"
...
g<Up>           1  等同於 "gk"
###'z' 開始的命令
z<CR>              重畫，游標移動到窗口的頂行的第一個非空白字元
z{height}<CR>      重畫，使得窗口高度為 {height} 行
z+                 游標移動到第 N 行 (預設為窗口之後的第一行)，其它同 "z<CR>"
z-                 重畫，游標移動到窗口末行的第一個非空白字元
z.                 重畫，游標移動到窗口的中間行的第一個非空白字元
z=                 給出拼寫建議
...
zW                 把單詞標記為拼寫錯誤 (wrong)
zX                 重新應用 'foldlevel'
z^                 游標移動到第 N 行 (預設為窗口之前的一行)，其它同 "z-"
za                 打開關閉的摺疊，或關閉打開的摺疊
zb                 重畫，游標在窗口底行
zc                 關閉摺疊
zd                 刪除摺疊
ze                 'wrap' 關閉時，水平滾動使得游標定位到屏幕的尾部 (最右側)
zf{motion}         為 Nmove 文本建立摺疊
zg                 把單詞標記為拼寫正確 (good)
zh                 'wrap' 關閉時，水平右滾屏幕 N 個字元
zi                 切換 'foldenable'
zj              1  移動到下一個摺疊的開始處
zk              1  移動到前一個摺疊的結束處
zl                 'wrap' 關閉時，水平左滾屏幕 N 個字元
zm                 從 'foldlevel' 減一
zn                 復位 'foldenable'
zo                 打開摺疊
zr                 給 'foldlevel' 加一
zs                 'wrap' 關閉時，水平滾動使得游標定位到屏幕的起始處 (最左側)
zt                 重畫，游標移動到窗口的頂部
zw                 把單詞標記為拼寫錯誤 (wrong)
zv                 打開足夠的摺疊，使得當前行可見
...
A               2  列塊模式下: 在高亮區域的所有行後附加相同的文本
C               2  刪除高亮行，並開始插入
D               2  刪除高亮行
I               2  列塊模式: 在所有高亮行之前插入相同的文本
J               2  連接高亮行
...
###命令行編輯
CTRL-@          不用
CTRL-A          使用游標前面的模式進行補全，並插入所有的匹配
CTRL-B          游標移動到命令行開始的地方
CTRL-C          等同於 <ESC>
CTRL-D          列出游標前面模式的所有補全
CTRL-E          游標移動到命令行結束的地方
CTRL-F          'cedit' 的預設值: 打開命令行窗口；否則不用
CTRL-G          不用
<BS>            刪除游標前面的字元
...
<S-Right>       游標右移一個單詞
<C-Right>       游標右移一個單詞
<Up>            從歷史中調入匹配游標前模式的前一個命令行
<S-Up>          從歷史中調入前一個命令行
<Down>          從歷史中調入匹配游標前模式的後一個命令行
<S-Down>        從歷史中調入後一個命令行
<Home>          游標移動到命令行開始的地方
<End>           游標移動到命令行結束的地方
<PageDown>      等同於 <S-Down>
